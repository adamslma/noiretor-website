  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/15 bg-[#191815]/90 text-[#f7f3ea] backdrop-blur-md">
      <div className="site-container flex h-[4.75rem] items-center justify-between">
          ...
          {links.map((link) => {
            ...
            return (
              ...
              >
                {link.label}
                {active && (
                  <span className="absolute inset-x-0 -bottom-[1.05rem] h-px bg-[#d0ad74]" />
                )}
              </Link>
            );
          })}
        ...
        <Link
          href="/reservation"
          className="hidden border border-[#d0ad74]/70 px-4 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.15em] hover:bg-[#d0ad74] hover:text-[#191815] md:inline-flex"
        >
          Une table
        </Link>
      ...
      >
        ...
        >
          {links.map((link, index) => (
            ...
            >
              {link.label}
              <span className="font-sans text-[0.65rem] text-white/35">0{index + 1}</span>
            </Link>
          ))}
        </nav>
      </div>
